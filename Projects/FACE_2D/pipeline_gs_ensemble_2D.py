...
from counterfactual_alignment.knowledge_functions import counterfactual_vector_paths, counterfactual_feasible_vector_paths
...
training_set = customDataset(datasets['train']['original'])
validation = customDataset(datasets['dev']['original'])
test_original = customDataset(datasets['test']['original'])
control = customDataset(datasets['control']['original'])

training_dataloader = data.DataLoader(training_set, batch_size=config['hyperparams']['batch_size'], shuffle=True, drop_last=False, collate_fn=ut.custom_collate_2D, generator=torch.Generator().manual_seed(config['hyperparams']['seed']))
...
test_original_data_loader = data.DataLoader(test_original, batch_size=config['hyperparams']['batch_size'], shuffle=True, drop_last=True, collate_fn=ut.custom_collate_2D)

control_data_loader = data.DataLoader(control, batch_size=config['hyperparams']['batch_size'], shuffle=True, drop_last=True, collate_fn=ut.custom_collate_2D)
...
n_vectors = len(training_set.X[0])
...
for epoch in tqdm(range(n_epochs)):
    ...
    train_metrics = ut.generate_results_ensemble(training_set.X,training_set.Y,models,ensemble_params,name="Train")
    val_metrics = ut.generate_results_ensemble(validation.X,validation.Y,models,ensemble_params,name="Validation")
    test_o_metrics = ut.generate_results_ensemble(test_original.X,test_original.Y,models,ensemble_params,name="Test Original")
    control_metrics = ut.generate_results_ensemble(control.X,control.Y,models,ensemble_params,name="Control")
    ...
    ensemble['outputs']['results']['Test Original'] = {'losses': [], 'accuracy': []}
    ...
    ensemble['outputs']['results']['Control'] = {'losses': [], 'accuracy': []}
    ensemble['outputs']['results']['Control']['losses'].append(control_metrics['loss'])
    ensemble['outputs']['results']['Control']['accuracy'].append(control_metrics['accuracy'])
...
if config['visualisation']['video']:
    ut.plotEpoch(training_set.X,training_set.Y,
          ensemble['models'][0],
          plot_states,
          K = training_set.K,
