...
from counterfactual_alignment.utilities import (visualise_classes, numpy_collate, imdb_collate,
                        ...
                        boundary_filter, save_stats, train_one_epoch, combine_datasets, select_informative_samples)
...
ensemble = {
            ...
            'models':[
                custom_models[config['hyperparams']['model']](
                    ...
                    config['hyperparams']['embedding_size'])
                    ]*n_models,
...
if False:
    ...
    for i in range(len(full_training_set)):
        if np.all(train_original.K['vector'][i]) == 0:
...
if active_sampling:
    nprng =np.random.default_rng(seed=123)
    init_samplesize = 50
...
output_name = (f"MODEL_ENSEMBLE_{n_models}_{model_name}__"
               f"vocab{config['hyperparams']['vocabulary_size']}_"
               f"emb{config['hyperparams']['embedding_size']}_"
               f"OPTIM_{optim_name}__LR_{learning_rate}__BATCHSIZE_{batch_size}__"
               f"DATA_{data_name}_trainsize_{train_size}__active_sampling_{active_sampling}_samplesize_{sample_size}_div{config['data_params']['diversity']}_"
...
for epoch in tqdm(range(n_epochs)):
    ...
    for m in range(n_models):
        ...
        for batch in train_original_data_loader:
            trained_state, train_metrics = train_one_epoch(trained_state, batch, model, loss_function,rng)
    ...
    if active_sampling and epoch%3==0 and epoch > 1 and len(training_set) < train_size:
