...
from counterfactual_alignment.utilities import (visualise_classes, mnist_collate_beta, select_informative_samples,
...
validation_data_loader = data.DataLoader(validation, batch_size=batch_size, shuffle=True, collate_fn=mnist_collate_beta, drop_last=True)

test_original = customDataset(datasets['test']['original'])
test_original_data_loader = data.DataLoader(test_original, batch_size=batch_size, shuffle=True, collate_fn=mnist_collate_beta, drop_last=True)
...
for epoch in tqdm(range(n_epochs)):
    ...
    if active_sampling and epoch%2==0 and epoch > 1 and len(training_set) < train_size:
        ...
        train_original_data_loader = data.DataLoader(training_set, batch_size=batch_size, shuffle=True, collate_fn=mnist_collate_beta, drop_last=True)
