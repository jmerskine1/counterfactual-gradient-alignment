...
# Refined Experiment Configuration
LOSS_FUNCTIONS = [
...
# Focusing around 0.5 where we saw good results
ALPHAS = [0.3, 0.5, 0.7] 
# Focusing on Size 50 (where we won) and 100 (where we were close)
TRAIN_SIZES = [50, 100]

CONFIG_PATH = 'Projects/MNIST_FACE/config.yaml'
RESULTS_FILE = 'Projects/MNIST_FACE/experiment_results_refined.json'
...
def run_pipeline():
    try:
        ...
        matches = re.findall(r'Validation | Loss: [\d\.]+, Accuracy: ([\d\.]+)%', output)
        if matches:
...
def main():
    ...
    print(f"Starting {len(all_experiments)} refined experiments with 5 epochs...")
