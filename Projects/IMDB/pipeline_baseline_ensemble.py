...
# Custom Libraries
from gradient_supervision_package.library.custom_models   import SimpleClassifier, MLP, CNN,  GSPaper, GSPaperNew, GSPaper2, GSPaper3, BagOfWordsClassifier, BagOfWordsClassifierSimple, BagOfWordsClassifierSingle
...
output_name = f'MODEL_ENSEMBLE_{model_name}__OPTIM_{optim_name}__LR_{learning_rate}__BATCHSIZE_{batch_size}__DATA_{data_name}_filtered__LOSS_{loss_name}'
...
train_original_data_loader = data.DataLoader(train_original, batch_size=batch_size, shuffle=True, collate_fn=custom_collate, drop_last=True)

train_combined = customDataset(combine_datasets(datasets['train']['original'],datasets['train']['modified']))
train_combined_data_loader = data.DataLoader(train_combined, batch_size=batch_size, shuffle=True, collate_fn=custom_collate, drop_last=False)
...
for epoch in tqdm(range(100)):
    ...
    for m in range(n_models):
        ...
        trained_state, train_metrics = train_one_epoch(trained_state, train_original_data_loader,  
    ...
    train_metrics = generate_results_ensemble(train_combined,models,ensemble_params,name="Train")
