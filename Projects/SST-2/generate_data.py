...
def compile_K(data,label, paired=False,cf=False,int_out=False, train=False, size = None):
    ...
    # print("X:",testarr)
    # print("K:",[sum(v[0]) for v in vector[:]])
    if train:
        diff = [sum(d) - sum(v[0]) for d, v in list(zip(data['vector'],vector))]
        idents = [i for i,t in enumerate(diff) if t==0]
        print(f"Found {len(idents)} identical vectors in {len(diff)} strings between original and counterfactual:" )
        
        data['vector'] = np.delete(data['vector'], idents, axis=0)
        label = np.delete(label, idents, axis=0)
        vector = np.delete(vector, idents, axis=0)
        labels = np.delete(labels, idents, axis=0)
        magnitude = np.delete(magnitude, idents, axis=0)
        ...
        # for arr in [data['vector'],vector,labels,magnitude]:
        #     arr = np.delete(arr, idents, axis=0)
    print(f"Returning {len(data['vector'])} samples with {len(vector)} counterfactuals")
    return {'text':np.array(data['text']),
            'X':np.array(data['vector']),
            'Y':label,
            # 'K':{'X':[np.array([d]) for i,d in enumerate(data['vector']) if i not in idents],
            #      'K':[np.array(v) for i,v in enumerate(vector) if i not in idents],
            #      'Y':[np.array(l) for i,l in enumerate(labels) if i not in idents],
            #      'magnitude':[np.array(m) for i,m in enumerate(magnitude) if i not in idents], 
            #      'text':[c for c in cf_text]}}
            'K':{'X':[np.array([d]) for d in data['vector']],
                 'K':[np.array(v) for v in vector],
                 'Y':[np.array(l) for l in labels],
                 'magnitude':[np.array(m) for m in magnitude], 
                 'text':[c for c in cf_text]}}
...
cf_train={'original': compile_K(cf_X,train_label,paired=True ,train=True, size=size),
         'modified': compile_K(cf_X,train_label,paired=True,cf=True,train=True, size=size)
         }
...
pickle_data = {'train':cf_train,'test':cf_test,'dev':cf_dev, 'n_classes':2}

embedding_path = os.path.join(base_dir,f'data/integer_len{max_length}_size{size}.pkl')
