...
output_name = f'MODEL_ENSEMBLE_{model_name}__OPTIM_{optim_name}__LR_{learning_rate}__BATCHSIZE_{batch_size}__DATA_{data_name}_filtered_{filter_info}__LOSS_{loss_name}_alpha_20'
...
for epoch in tqdm(range(50)):
    ...
    train_metrics = generate_results_ensemble(training_data.X['vector'],training_data.Y,models,ensemble_params,name="Train")
    val_metrics = generate_results_ensemble(validation.X['vector'],validation.Y,models,ensemble_params,name="Validation")
    test_o_metrics = generate_results_ensemble(test_original.X['vector'],test_original.Y,models,ensemble_params,name="Test Original")
    test_m_metrics = generate_results_ensemble(test_modified.X['vector'],test_modified.Y,models,ensemble_params,name="Test Modified")
