...
output_name = f'MODEL_ENSEMBLE_{n_models}_{model_name}__OPTIM_{optim_name}__LR_{learning_rate}__BATCHSIZE_{batch_size}__DATA_{data_name}_filtered__LOSS_{config["hyperparams"]["loss_function"]}_mix_{config["hyperparams"]["loss_mix"]}'
...
for epoch in tqdm(range(n_epochs)):
    ...
    for m in range(n_models):
        ...
        for batch in train_combined_data_loader:
            trained_state, train_metrics = train_one_epoch(trained_state, batch, model, loss_functions[config["hyperparams"]["loss_function"]],rng)
